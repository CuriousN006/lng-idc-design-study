...
def _evaluate_fluid_candidate(task: dict[str, object]) -> dict[str, object]:
    config = task["config"]
...
def compute_fluid_screening(
    config: dict,
    required_cooling_kw: float,
    parallel_options: ParallelOptions | None = None,
) -> dict[str, object]:
    candidates = config["coolant_candidates"]
    options = parallel_options or ParallelOptions(enabled=False, workers=1)
    rows = map_items(
        _evaluate_fluid_candidate,
        [
            {
                "config": config,
