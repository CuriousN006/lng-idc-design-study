...
def compute_baseline_cycle(config: dict, total_cooling_kw: float) -> dict[str, object]:
    ...
    p_low = props_si("P", "T", evaporating_temp_k, "Q", 1.0, fluid)
    p_high = props_si("P", "T", condensing_temp_k, "Q", 0.0, fluid)

    h1 = props_si("H", "T", evaporating_temp_k, "Q", 1.0, fluid)
    s1 = props_si("S", "T", evaporating_temp_k, "Q", 1.0, fluid)
    h2s = props_si("H", "P", p_high, "S", s1, fluid)
    h2 = h1 + (h2s - h1) / compressor_eta
    h3 = props_si("H", "T", condensing_temp_k, "Q", 0.0, fluid)
    h4 = h3
