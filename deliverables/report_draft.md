...
본 연구는 2022년 열시스템디자인 과제를 재현 가능한 코드 기반 설계 연구로 다시 구성한 결과물이다. 대상 IDC의 총 냉방부하는 **13,476.0 kW**로 계산되었고, 이론 최소동력은 **1,215.8 kW**, 기준 R-134a 증기압축 시스템의 압축기 동력은 **4,185.4 kW**로 산정되었다. 이에 비해 LNG 냉열 기반 시스템은 **R-717 (Ammonia)**를 2차 루프 냉각유체로 사용할 때 LNG 기화 duty **18,010.5 kW**, 배관 열유입 **580.4 kW**, LNG 순환펌프 **22.2 kW**와 IDC 2차 루프 펌프 **123.7 kW**를 포함한 핵심 전동부하 **145.9 kW**의 supplemental warm-up을 포함한 하이브리드 운전 기준 설계가 가능했다. 그 결과 기준 시스템 대비 전력 절감은 **4,039.6 kW**, 연간 전력 절감은 **35,386.6 MWh/년**, 연간 비용 절감은 **3,717.4 백만원/년**, 연간 회피 배출은 **16,224.7 tCO2/년**으로 평가되었다. 대표 혼합 LNG는 **Peak-shaving LNG surrogate**으로 모델링했고, 10 km 기본안 기준 핵심 설비 CAPEX는 **66.32 십억원**, NPV는 **-42.85 십억원**으로 추정되었다. 현재 운전점에서의 추정 최대 하이브리드 성립거리는 약 **103.8 km**, 기본 LNG duty 기준 최대 성립거리는 약 **0.0 km**이며, 35 km 조건은 **하이브리드 성립, 기본 LNG duty 불성립**으로 판정되었다.
...
| 거리 (km) | Core system power (kW) | 열유입 (kW) | 추가 warm-up (kW) | 기본 duty 여유 (kW) | 하이브리드 성립 | 기본 duty 충족 |
| --- | --- | --- | --- | --- | --- | --- |
...
거리 증가에 따라 열유입이 거의 선형적으로 증가하면서, 현재 운전점에서의 추정 최대 편도 성립거리는 **103.8 km**로 나타났다. 따라서 **35 km** 조건은 기본안의 단순 확장만으로는 **하이브리드 성립, 기본 LNG duty 불성립 판정**을 받는다.

### 4.5 공급온도 민감도와 35 km 해석

| 공급온도 (°C) | 선정 유체 | Core system power (kW) | 최대 하이브리드 성립거리 (km) | 최대 기본 duty 성립거리 (km) | 35 km 기본 duty 충족 | 상태 |
| --- | --- | --- | --- | --- | --- | --- |
