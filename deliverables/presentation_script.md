...
- 추정 최대 하이브리드 편도 운전 가능거리는 약 103.8 km다.
- 기본 LNG duty 기준 최대 편도 성립거리는 약 0.0 km다.
- 따라서 현재 설계점에서 35 km 조건은 하이브리드 운전은 가능하지만 기본 LNG duty는 불성립이라고 정리한다.
...
- LNG 외부 루프 펌프는 22.2 kW, IDC 2차 루프 펌프는 123.7 kW이며, 합산 core system power는 145.9 kW라고 설명한다.
- 만약 보조 열원이 남는다면, 현재 시나리오 중 최선은 Waste-heat recovery loop이고 총 시스템 동력은 195.4 kW라고 설명한다.

## 슬라이드 16. 열역학/경제성 평가 - 연간 효과와 회수기간
- 연간 전력 절감량은 35,386.6 MWh/년이다.
- 연간 전력요금 절감은 3,717.4 백만원/년 수준이다.
- 연간 회피 간접배출은 16,224.7 tCO2/년이다.
- 다만 core installed CAPEX는 66.32 십억원, NPV는 -42.85 십억원으로 현재는 투자 부담이 크다고 덧붙인다.
- 보조 열원까지 포함한 재무 최선 시나리오는 Ambient-air trim heater이고, 해당 시나리오 NPV는 -44.93 십억원이다.
...
- 기준 proxy Methane 대비 기화기 요구 면적 변화는 0.00%에서 91.87% 범위라고 설명한다.
- IDC 2차 루프는 네트워크 보수화 시 펌프동력이 154.8 kW까지 증가할 수 있다고 설명한다.
- 현재 기본안에서 35 km 조건은 하이브리드는 성립하지만 기본 LNG duty는 아직 불성립이라고 정리하고, 그 이유를 IDC 측 HX와 총 duty 관점에서 설명한다.
...
## 슬라이드 19. 결론
- 10 km 기본안은 supplemental warm-up을 포함한 하이브리드 운전 기준으로 기술적으로 성립한다고 결론낸다.
- 동시에 35 km 조건은 현재 기본안에서 하이브리드 운전은 가능하지만 기본 LNG duty는 불성립이며, 추정 최대 하이브리드 편도 운전 가능거리는 약 103.8 km라고 정리한다.
- 공급온도 스윕 최적점은 -63.1 °C / R-717 (Ammonia)라고 덧붙인다.
- 현실성 필터 결과 채택 가능한 무보조 해는 아직 없다고 정리한다.
